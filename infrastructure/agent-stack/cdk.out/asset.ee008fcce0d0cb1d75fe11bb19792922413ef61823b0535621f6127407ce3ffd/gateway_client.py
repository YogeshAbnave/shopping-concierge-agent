...
def get_gateway_access_token() -> str:
    ...
    try:
        ...
        # Get Cognito domain from CloudFormation export
        deployment_id = os.environ.get("DEPLOYMENT_ID", "default")
        try:
            # Try to get domain from CloudFormation export first
            cf = boto3.client("cloudformation", region_name=region)
            exports = cf.list_exports()["Exports"]
            domain_export = next(
                (exp for exp in exports if exp["Name"] == f"ConciergeAgent-{deployment_id}-Auth-Domain"),
                None
            )
            if domain_export:
                domain = domain_export["Value"]
            else:
                # Fallback: construct domain name based on deployment pattern
                sts = boto3.client("sts", region_name=region)
                account_id = sts.get_caller_identity()["Account"]
                domain = f"concierge-{deployment_id}-{account_id}"
        except Exception as e:
            logger.warning(f"Could not get domain from CloudFormation: {e}")
            # Final fallback: construct domain name
            sts = boto3.client("sts", region_name=region)
            account_id = sts.get_caller_identity()["Account"]
            domain = f"concierge-{deployment_id}-{account_id}"
...
def get_gateway_client(tool_filter_pattern: str, prefix: str = "gateway") -> MCPClient:
    ...
    Args:
        tool_filter_pattern: Regex pattern to filter tools (e.g., "^carttools")
    ...
    Example:
        cart_client = get_gateway_client("^carttools")
    """
    import re
    ...
    try:
        ...
        # Create client with connection timeout and retry settings
        client = MCPClient(
        ...
        logger.error(f"❌ Failed to create Gateway MCP client: {e}")
        # Return a mock client that will fail gracefully
        raise ConnectionError(f"Gateway MCP client unavailable: {e}")
