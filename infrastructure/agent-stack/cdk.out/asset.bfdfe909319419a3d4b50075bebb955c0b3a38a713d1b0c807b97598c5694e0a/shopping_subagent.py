    ...
    try:
        ...
        if any(term in query.lower() for term in ['water bottle', 'find', 'search', 'show me', 'get me']):
            ...
            try:
                ...
                tools = await shopping_client.list_tools()
                search_tool = None
                for tool in tools:
                    if tool.name == "single_productsearch":
                        search_tool = tool
                        break
                
                if search_tool:
                    result = await shopping_client.call_tool(
                        "single_productsearch",
        ...
        **RESPONSE REQUIREMENTS**:
        ...
        try:
            ...
            if "toolResult blocks" in error_msg and "toolUse blocks" in error_msg:
                ...
                simple_result = f"I understand you're looking for water bottles. Let me search for some options for you."
